...
$vf=[];$af=[];$n=count($norm);$font=defined('FFMPEG_FONT')?FFMPEG_FONT:((getenv('WINDIR')?:'C:/Windows').'/Fonts/arial.ttf');
...
foreach($norm as $i=>$s){[$a,$b]=$s;$dur=$b-$a;$filters="trim=start=$a:end=$b,setpts=PTS-STARTPTS,scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920";
 foreach($texts as $t){$ts=(float)($t['start']??-1);$te=(float)($t['end']??-1);$raw=(string)($t['text']??'');if($raw===''||$te<=$a||$ts>=$b)continue;$relS=max(0,$ts-$a);$relE=min($dur,$te-$a);if($relE<=$relS)continue;$txt=filter_escape($raw);$size=max(18,min(180,(int)($t['size']??64)));$color=preg_match('/^#[0-9a-fA-F]{6}$/',(string)($t['color']??''))?$t['color']:'#ffffff';$filters.=",drawtext=fontfile='".str_replace('\\','/',$font)."':text='".$txt."':fontcolor=$color:fontsize=$size:borderw=3:bordercolor=black@0.75:x=(w-text_w)/2:y=".text_y($t).":enable='between(t,$relS,$relE)'"; }
 $vf[]="[0:v]$filters[v$i]";
